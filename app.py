        ...
        with tab3:
            if not exams_list:
                ...
                for idx, exam in enumerate(exams_list):
                    subj = exam.get('subject', 'غير محدد')
                    tech = exam.get('teacher', 'غير محدد')
                    with st.expander(f"📋 أسئلة مادة ({subj}) - التدريسي: {tech}"):
                        content_text = exam.get('content', 'لا يوجد محتوى')
                        exam_type_val = exam.get('exam_type', 'النهائي')
                        stage_val = exam.get('stage', '-')
                        time_val = exam.get('time', '-')
                        day_val = exam.get('day', '-')
                        date_val = exam.get('date', '-')
                        head_val = exam.get('head', '-')
                        
                        # تم إصلاح القوس والإغلاق بشكل برمي آمن هنا لتعمل الفورمة الرسمية فوراً
                        st.markdown(f"""
                        <div class="print-area">
                            <div class="main-header">
                                ...
                                <h5 style="margin:5px; color:black;">أسئلة امتحان {exam_type_val}</h5>
                                <hr style="border:1px solid black; margin:10px 0;">
                                <table style="width:100%; text-align:right; border-collapse:collapse; color:black; font-weight:bold;">
                                    <tr>
                                        <td>المرحلة: {stage_val}</td>
                                        <td>المادة: {subj}</td>
                                        <td>الوقت: {time_val}</td>
                                    </tr>
                                    <tr>
                                        <td>اليوم: {day_val}</td>
                                        <td>التاريخ: {date_val}</td>
                                        <td>رئيس القسم: {head_val}</td>
                                    </tr>
                                </table>
                            </div>
                            <div style="white-space: pre-wrap; font-size:18px; padding:15px; border:1px solid #ccc; border-radius:5px; background:#fff; color:black; line-height:1.8; text-align:right;">{content_text}</div>
                            <br>
                            <p style="text-align:left; font-weight:bold; color:black; padding-left:20px;">مدرس المادة: {tech}</p>
                        </div>
                        """, unsafe_allow_html=True)
